...
limited_data <- ranked_measure[c('FIPS', 'State', 'County', 'Preventable Hospitalization Rate', '% Uninsured', '# Primary Care Physicians', 'Primary Care Physicians Rate', 'Primary Care Physicians Ratio', '% Adults with Obesity')]
view(limited_data)
...
bystate<- renamed %>%
  ...
  summarise(mean_preventable_hr_by_state = mean(preventable_hr, na.rm = TRUE), 
            ...
            mean_adult_obesity = mean(pct_adult_obesity, na.rm = TRUE))
bystate
...
model5<- lm(preventable_hr ~ pct_uninsured + num_ratio_primary_cp + num_primary_cp + rate_primary_cp + state, data = renamed)
stargazer(model1, model2, model3, model4, model5, type = "html", out = "regression.html" ,title = "My regression models")
model6<- lm(preventable_hr ~ state, data = renamed)
summary(model6)
model7<- lm(preventable_hr ~ state + pct_uninsured, data = renamed)
summary(model7)
